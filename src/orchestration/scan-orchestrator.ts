...
export class ScanOrchestrator implements IScanOrchestrator {
  ...
  public async *scan(input: string | EngineInput): AsyncGenerator<ScanProgress, void, unknown> {
    ...
    const isWorkerDispatch =
      Boolean(this.workerDispatcher) && text.length > this.workerThreshold;
    ...
    if (isWorkerDispatch && this.workerDispatcher) {
      yield {
        status: 'scanning',
        stage: 'Dispatching regex/entropy analysis to computation worker',
        percentage: 10,
        findings: [],
        layerStatuses: snapshotLayerStatuses(layerStatuses),
      };
      ...
      const workerOutcome = await Promise.race([
        this.workerDispatcher
          .analyze(text, { signal })
          .then((result) => ({ type: 'done' as const, result })),
      ...
      if (
        ...
        this.isAborted ||
        workerOutcome.type === 'aborted'
      ) {
        yield abortYield([], layerStates, 10, 'Scan aborted during worker dispatch');
        this.abortController = null;
        this.abortNotify = null;
        return;
      }
      ...
      if (result.fellBackToMainThread) {
        yield {
          status: 'scanning',
          stage: 'Scanning in compatibility mode',
          percentage: 20,
          findings: [],
          layerStatuses: snapshotLayerStatuses(layerStatuses),
          degradationWarning:
            'Computation worker unavailable — scanning in compatibility mode on the main thread.',
          note: 'DEGRADATION_WARNING: worker fallback',
        };
      }

      const regexFindings = result.findings.filter((f) => f.detectionLayer === 1);
      const entropyFindings = result.findings.filter((f) => f.detectionLayer === 3);
      // Findings without layer metadata still count toward the aggregate.
      const otherFindings = result.findings.filter(
        (f) => f.detectionLayer !== 1 && f.detectionLayer !== 3
      );
      accumulatedFindings.push(...regexFindings, ...entropyFindings, ...otherFindings);

      layerStates['Layer 1 (Regex)'] = layer1Engine ? 'completed' : 'skipped';
      layerStates['Layer 3 (Entropy)'] = layer3Engine ? 'completed' : 'skipped';
      updateLayerStatus(layerStatuses, 'regex', {
        status: 'complete',
        findings: [...regexFindings],
      });
      updateLayerStatus(layerStatuses, 'entropy', {
        status: 'complete',
      ...
      yield {
        status: 'scanning',
        stage: result.usedWorker
          ? 'Worker regex/entropy analysis complete'
          : 'Main-thread regex/entropy analysis complete',
        percentage: 66,
        findings: [...accumulatedFindings],
        layerStatuses: snapshotLayerStatuses(layerStatuses),
        ...(result.fellBackToMainThread
          ? {
              degradationWarning:
                'Computation worker unavailable — scanning in compatibility mode on the main thread.',
            }
